...
class Exp_crossformer(Exp_Basic):
    ...
    def train(self, setting):
        ...
        if self.args.skip_testing != True:
        ...
        writer = SummaryWriter('runs/'+setting)
        ...
        for epoch in range(self.args.train_epochs):
            ...
            for i, (batch_x,batch_y) in enumerate(train_loader):
                ...
                writer.add_scalar("Loss/Minibatches", loss.item(), epoch*train_steps + i)
            ...
            writer.add_scalar("TrainLoss/Epochs", train_loss, epoch+1)
            writer.add_scalar("ValiLoss/Epochs", vali_loss, epoch+1)
            
            if self.args.skip_testing != True:
                test_loss = self.vali(test_data, test_loader, criterion)
                writer.add_scalar("TestLoss/Epochs", test_loss, epoch+1)
    ...
    def test(self, setting, save_pred = False, inverse = False):
        ...
        # result save
        folder_path = os.path.join(args.work_folder, 'results', setting)
    ...
    def generate(self, setting, save_pred = False, inverse = False):
        args = self.args
        data_set = Dataset_MTS(
            root_path=args.root_path,
            data_path=args.data_path,
        ...
        with torch.no_grad():
            ...
            while count < args.sample_size:
                # print("batch_x.shape: ", batch_x.shape) # ([1, 168, 17])                    
                # batch_x: 128 --> batch_y: 24 
                # print(batch_x.shape) # ([1, 168, 17])
                pred, true = self._process_one_batch(
                    data_set, batch_x, batch_y, inverse)
                # print(pred.shape) # ([1, 24, 17])
                batch_size = pred.shape[0]
                instance_num += batch_size
                batch_metric = np.array(metric(pred.detach().cpu().numpy(), true.detach().cpu().numpy())) * batch_size
                metrics_all.append(batch_metric)
                if (save_pred):
                    preds.append(pred.detach().cpu().numpy())
                    trues.append(true.detach().cpu().numpy()) # it has no meaningful true values in this case
                    
                count = count + args.out_len
                batch_x = np.concatenate((batch_x.cpu()[:, args.out_len:, :], pred.cpu()[:, :, :]), axis=1)
        ...
        if (save_pred):
            ...
            print(preds[:, :args.sample_size, :].shape)
            trues = np.concatenate(trues, axis = 1)
            np.save(os.path.join(folder_path, 'pred.npy'), preds[:, :args.sample_size, :])
