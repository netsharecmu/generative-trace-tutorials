...
class NetflowFormatTransformer(object):
    ...
    def rev_transfer(self, df, this_ip=None):
        ...
        try:
            b_max = self.dataset_stat["pkt_len_max"]
        except:
            pass
        tos_max = self.dataset_stat["tos_max"]
        id_max = self.dataset_stat["id_max"]
        off_max = self.dataset_stat["off_max"]
        ttl_max = self.dataset_stat["ttl_max"]
        ...
        # print(df.head()) 
        df['raw_scale_pkt_len'] = np.exp(df[2]*b_max)

        buffer = []
        for index, row in df.iterrows():
            line = [int(row[0]*24), row[1]*teDeltamax, int(row['raw_scale_pkt_len']), int(row[3]*tos_max), int(row[4]*id_max), int(row[5]*off_max), int(row[6]*ttl_max)]
            # line = [int(row[0]*24), row[1]*teDeltamax, int(row['raw_scale_byt']*b_max), int(row['raw_scale_pkt']*pktmax), row[4]*tdmax]
            if row[11] == 1:
                line.append(self.rev_port(row[7])) # sp
                line.append(self.rev_port(row[8])) # dp
                line.append(this_ip) #sa
                line.append('.'.join([str(int(da_i*256)) for da_i in row[9:9+4]])) #da
            else:
                line.append(self.rev_port(row[8])) # dp
                line.append(self.rev_port(row[7])) # sp
                line.append('.'.join([str(int(da_i*256)) for da_i in row[9:9+4]])) #da
        ...
        out_df.columns = ['hour', 'time_delta', 'pkt_len', 'tos', 'id', 'off', 'ttl', 'sp', 'dp', 'sa', 'da', 'pr', 'flag']

        return out_df
   
    def transfer(self, df):
        df['log_pkt_len'] = np.log(df['pkt_len'])
        df = df.replace([np.inf, -np.inf], 0)
        teTmax = self.dataset_stat["teT_max"]
        teDeltamax = 1.0 # df['teDelta'].max()
        try:
            teDeltamax = self.dataset_stat["teDelta_max"]
            if teDeltamax == 0:
        ...
        for index, row in df.iterrows():
            ...
            normlogbyt = row['log_pkt_len']/b_max
            if np.isinf(normlogbyt):
                normlogbyt = (10**9 * row['log_pkt_len']) / (10**9 * b_max)
            line = [row['teT']/teTmax, normteDelta, normlogbyt, row["tos"]/tos_max, row["id"]/id_max, row["off"]/off_max, row["ttl"]/ttl_max]
            label_line = [row['teT']/teTmax, normteDelta, normlogbyt, row["tos"]/tos_max, row["id"]/id_max, row["off"]/off_max, row["ttl"]/ttl_max]
