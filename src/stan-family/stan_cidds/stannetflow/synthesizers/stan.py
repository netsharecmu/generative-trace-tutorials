...
class NetflowFormatTransformer(object):
    ...
    def rev_transfer(self, df, this_ip=None):
        ...
        for index, row in df.iterrows():
            ...
            if row[13] == 1:
                ...
                line.append(prt[2])
            arrackType = ['---', 'portScan', 'dos', 'pingScan']
            if row[16] == 1:
                line.append(arrackType[0])
            elif row[17] == 1:
                line.append(arrackType[1])
            elif row[18] == 1:
                line.append(arrackType[2])
            else:
                line.append(arrackType[3])
            c_class = ['normal', 'victim', 'attacker']
            if row[20] == 1:
                line.append(c_class[0])
            elif row[21] == 1:
                line.append(c_class[1])
            else:
                line.append(c_class[2])
            buffer.append(line)
        out_df = pd.DataFrame(buffer)
        out_df.columns = ['hour', 'time_delta', 'byt', 'pkt', 'time_duration', 'sp', 'dp', 'sa', 'da', 'pr', 'type', 'label']
    ...
    def transfer(self, df):
        ...
        for index, row in df.iterrows():
            ...
            if "---" in row['type']:
                line_type = [1, 0, 0, 0]
            elif "portScan" in row['type']:
                line_type = [0, 1, 0, 0]
            elif "dos" in row['type']:
                line_type = [0, 0, 1, 0]
            elif "pingScan" in row['type']:
                line_type = [0, 0, 0, 1]
            line += line_type
            ...
            if "normal" in row['label']:
                class_type = [1, 0, 0]
            elif "victim" in row['label']:
                class_type = [0, 1, 0]
            else:
                class_type = [0, 0, 1]
            line += class_type
            label_line += class_type
