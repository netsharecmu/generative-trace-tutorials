# Supervised PatchTST (ICLR 2023) & DLinear -- pcap/netflow training/generation

## 1. pre-process from root folder 
## 2. train/generation together
* if you want to change the configuration (e.g., data path, input/output length, sample_size), please modify each shell script.
* Configurations 
    - `is_training` is whether you will train or generate
    - `root_path` and `data_path` is for the dataset path 
    - `model_name` is whether PatchTST or DLinear 
    - `enc_in` is # of cols except for `date` feature. (If you pre-processed with OHE, the # of cols should be based on the converted csv file.)
    - `seq_len` is # of rows for prediction input
    - `pred_len` is # of rows for prediction output
    - `target` is the target col name 
    - `skip_testing` is whether skip testing or not
    - `data_split` is the portion of train/vali/test dataset
    - the output of prediction will be stored in `results/` 
    - the model will be stored in `checkpoints/` 
    - runtime log is under `logs/`

* DLinear 
```
sh scripts/Linear/pcap.sh
sh scripts/Linear/netflow.sh
```
* Supervised PatchTST
```
sh scripts/PatchTST/pcap.sh
sh scripts/PatchTST/netflow.sh
```
