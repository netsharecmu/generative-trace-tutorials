...
class Exp_ETTh(Exp_Basic):
    ...
    def _get_data(self, flag):
        ...
        data_dict = {
            ...
            'ugr16': Dataset_Custom,
            'cidds': Dataset_Custom,
            'ton': Dataset_Custom,
            'caida': Dataset_Custom,
            'dc': Dataset_Custom,
            'ca': Dataset_Custom,
            'm57': Dataset_Custom,
        }
    ...
    def train(self, setting):
        ...
        if not self.args.skip_testing: 
            test_data, test_loader = self._get_data(flag = 'test')
        path = os.path.join(self.args.checkpoints, setting)
        print(path)
        if not os.path.exists(path):
            os.makedirs(path)
        writer = SummaryWriter(os.path.join(self.args.work_folder, 'event/{}'.format(self.args.model_name)))
    ...
    def generate(self, setting, generate=False):
        test_data, test_loader = self._get_data(flag='generation')
        ...
        # print("first, shape of batch_x", batch_x.shape) #(1, 48, 13)

        while count < self.args.sample_size:
            pred, pred_scale, mid, mid_scale, true, true_scale = self._process_one_batch_SCINet(
                test_data, batch_x, batch_y)
            
            # print("shape of pred", pred.shape) #(1, 24, 13)
            ...
            count = count + self.args.pred_len
            
            batch_x = np.concatenate((batch_x[:, self.args.pred_len:, :].detach().numpy(), pred[:, :, :].detach().numpy()), axis=1)
            batch_x = torch.from_numpy(batch_x)
            # print("concatenated, shape of batch_x", batch_x.shape) #(1, 48, 13)
            if count % 1000 == 0:
        ...
        if self.args.stacks == 1:
            ...
            # print("shape of pred_scales", pred_scales.shape)
        ...
        # result save
        if self.args.save:
            folder_path = os.path.join(self.args.work_folder, 'results', setting)
            if not os.path.exists(folder_path):
                os.makedirs(folder_path)
            ...
            pred_scales = pred_scales.reshape(1, preds.shape[0] * preds.shape[1], preds.shape[2])

            print("shape of preds", preds.shape)
            print("shape of pred_scales", pred_scales.shape)

            np.save(os.path.join(folder_path, 'metrics.npy'), np.array([mae, mse, rmse, mape, mspe]))
            np.save(os.path.join(folder_path, 'pred.npy'), preds[:self.args.sample_size, :])
            np.save(os.path.join(folder_path, 'true.npy'), trues)
            np.save(os.path.join(folder_path, 'pred_scales.npy'), pred_scales[:self.args.sample_size, :])
            np.save(os.path.join(folder_path, 'true_scales.npy'), true_scales)
