...
class Learner(GetAttr):
    ...
    def all_batches(self, type_):
        ...
        if type_ == 'generate': 
            ...
            while num < self.sample_size:
                self.iter, self.batch = num, batch            
                self.batch_generate()
                num = num + self.pred.shape[1]
    ...
    def _do_batch_generate(self):   
        self.pred = self.generate_step(self.batch)     
        
    def generate_step(self, batch):
        # get the inputs
        self.xb, self.yb = batch
        if self.pred is not None: 
            pred_len = self.pred.shape[1]
            self.xb = torch.cat([self.xb[:, pred_len:, :], self.pred], dim=1)
        print(self.xb.shape)
        # forward
        pred = self.model_forward()
    ...
    def generate(self, dl, weight_path=None, scores=None):
        """_summary_
        Args:
            test_data can be a tensor, numpy array, dataset or dataloader
        Returns:
        ...
        self('before_test')
        self.model.eval()
        with torch.no_grad(): self.all_batches('generate')
        self('after_test')   
        self.preds, self.targets = to_numpy([cb.preds, cb.targets])
        # calculate scores
        if scores: 
