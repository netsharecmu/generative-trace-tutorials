...
parser.add_argument('--is_generate', type=int, default=0, help='do AR generate or not')
parser.add_argument('--sample_size', type=int, default=100, help='size of generated data')
...
def test_func(weight_path):
    ...
    np.save(args.save_path+'pred.npy', out[0])
    np.save(args.save_path+'true.npy', out[1])
...
def generate_func(weight_path):
    ...
    cbs = [RevInCB(dls.vars, denorm=True)] if args.revin else []
    cbs += [PatchCB(patch_len=args.patch_len, stride=args.stride)]
    learn = Learner(dls, model,cbs=cbs, sample_size=args.sample_size)
    out  = learn.generate(dls.test, weight_path=weight_path+'.pth', scores=[mse,mae])         # out: a list of [pred, targ, score]
    print('score:', out[2])
    print('before: the shape of pred ', out[0].shape)
    
    preds = np.reshape(out[0], (1, out[0].shape[0] * out[0].shape[1], -1))
    print('after: the shape of pred:', preds[:,:args.sample_size,:].shape)

    np.save(args.save_path+'pred.npy', preds[:,:args.sample_size,:])
    np.save(args.save_path+'true.npy', out[1])
...
if __name__ == '__main__':
    ...
    if args.is_finetune:
        ...
        # TODO: change the initial start token to generate based on the exp plan
        out = generate_func(weight_path)        
        print('----------- Complete! -----------')
