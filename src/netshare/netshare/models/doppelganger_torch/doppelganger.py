...
class DoppelGANger(object):
    ...
    def _iteration_or_epoch(self, num_samples):
        '''
            Fix self.epochs when self.iterations is given and greater than sepcified epochs.

            num_samples: number of samples in the training data
        '''
        if self.iterations is not None and self.epochs is not None:
            target_epochs = math.ceil(self.iterations * self.batch_size / num_samples)
            if target_epochs > self.epochs:
    ...
    def train(
            self, epochs, data_feature, data_attribute, data_gen_flag,
            iterations=None, max_train_time=None):
        self.epochs = epochs
        ...
        self._iteration_or_epoch(len(dataset))
        self._train(dataset)
    ...
    def _train(self, dataset):
        ...
        for epoch in tqdm(range(self.epochs)):
            with open(self.time_path, "a") as f:
                epoch_start_time = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S.%f")
                f.write("epoch {} starts: {}\n".format(epoch, epoch_start_time))
            ...
            for batch_idx, (real_attribute, real_feature) in enumerate(loader):
                ...
                if self.iterations is not None: # if iterations is not None, then use iterations as the stopping criteria
                    if iteration >= self.iterations:
                        print(f"Reach the maximum number of iterations={self.iterations}!")
                        max_iteration_reached = True
                        break
            
            # always save the first epoch, last epoch, last iteration if max_iteration is reached
            if (epoch + 1) % self.epoch_checkpoint_freq == 0 or \
                epoch == 0 or \
                epoch == self.epochs - 1 or \
            ...
            with open(self.time_path, "a") as f:
                epoch_end_time = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S.%f")
                f.write("epoch {} ends: {}\n".format(epoch, epoch_end_time))
            
            if max_iteration_reached:
                break

            # if max train time is reached, save the model and break the training loop
            if self.max_train_time is not None and (time.time() - start_time_train > self.max_train_time):
                print(f"Reach the maximum training time={self.max_train_time}!")
                ckpt_path = os.path.join(
                    self.checkpoint_dir, f"epoch_id-{epoch}.pt")
                self.save(ckpt_path)
                break
