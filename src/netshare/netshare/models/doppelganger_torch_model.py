...
class DoppelGANgerTorchModel(Model):
    def _train(self, input_train_data_folder, output_model_folder, log_folder):
        ...
        dg.train(
            ...
            max_train_time=self._config["max_train_time"]
        )
    ...
    def _generate(self, input_train_data_folder,
        ...
        num_samples = (data_attribute.shape[0] if ((given_data_attribute is None) and (given_data_attribute_discrete is None)) else given_data_attribute.shape[0])

        # TEMP FIX: assume that num_samples == # of samples in the real data
        ...
        #     )
        # ) + [self._config["epochs"] - 1] # include the first and last epoch
        # # reverse list in place
        epoch_range = []
        for ckpt_file in os.listdir(checkpoint_dir):
            if not ckpt_file.endswith(".pt"):
                continue
            ckpt_file_basename = os.path.splitext(os.path.basename(ckpt_file))[0]
            epoch_id = int(ckpt_file_basename.split("-")[1])
            epoch_range.append(epoch_id)
        epoch_range = sorted(epoch_range)
        epoch_range.reverse()
