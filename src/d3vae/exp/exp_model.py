...
class Exp_Model(Exp_Basic):
    ...
    def train(self, setting):
        ...
        for epoch in range(self.args.train_epochs):
            ...
            if not self.args.skip_testing:
                ...
                print("vali_mse:{0:.7f}".format(vali_mse))
    ...
    def generate(self, setting):
        ...
        self.denoise_net.load_dict(paddle.load(model_path))
        
        test_data, test_loader = self._get_data(flag='generate')
        preds = []
        trues = []
        noisy = []
        input = []
        ...
        while count < self.args.sample_size:
            ...
            # print('batch_x shape:', batch_x.shape, batch_x_mark.shape)
            ...
            count = count + self.args.prediction_length
            batch_x = np.concatenate((batch_x[:, self.args.prediction_length:, :], out[0,:, :, :]), axis=1)
        ...
        np.save(os.path.join(folder_path, 'noisy.npy'), noisy)
        np.save(os.path.join(folder_path, 'true.npy'), trues)
        np.save(os.path.join(folder_path, 'input.npy'), input)
        return mae, mse
